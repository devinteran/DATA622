...
```{r}
lda.fit <- train(Loan_Status ~ ApplicantIncome + LoanAmount,
             data = train,
             method = 'lda',
             trControl = ctrl
             )
...
\* Other variables used in the decision tree include LoanAmount, PropertyArea
...
Now we will use our model to see how it performs on the training data. We see that the model predicted **Loan\_Status** with an accuracy of 83%. 78 instances were incorrectly classified.
...
|:-----------------:|:------:|:------------------------:|:--------------:|:-------------:|
|      Accuracy     | 0.8374 |          0.7869          |     0.8293     |     0.8374    |
| Balanced Accuracy | 0.7066 |          0.6795          |     0.7010     |     0.7066    |
|    Sensitivity    | 0.4242 |          0.3947          |     0.4242     |     0.4242    |

: Model Performance Matrix

Notice that the sensitivity among LDA, Decision Trees, and RF is the same at 42.42%. It's surprising to see that LDA and Random Forest ended up having the highest accuracy, which is usually the go-to metric to go for in an unbalanced dataset with the binary response that are not 50/50.
